...
n_elements = 1000  # specify number of elements
...
for l,t in enumerate(ts): 
    ...
    # Number the remaining nodes from 0 to n_equations-1
    count = 0
    ...
    M = scipy.sparse.coo_matrix((vals,(rows,cols)), shape=(n_equations,n_equations)).tocsr()
 
    # Compute finite element solution
    ua = np.zeros(n_nodes)
    ua[i_dir] = ue(x[i_dir],t)           # apply prescribed values at Dirichlet nodes
    ua[ide >=0] = linalg.spsolve(M,c)    # solve the system at interior nodes
