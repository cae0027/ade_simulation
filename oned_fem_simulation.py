...
def oned_linear(load, test, w_g):
    """
    Compute the one-dimensional local linear form (f,phi)
    
    Inputs:
    
        load: double (n_gauss,) kernel evaluated at the Gauss nodes.
...
#     K = np.dot(np.dot(test_x.T,np.diag(kernel_K*w_g)),phi_x)
#     W = np.dot(np.dot(test.T,np.diag(kernel_W*w_g)),phi_x)
    return M
